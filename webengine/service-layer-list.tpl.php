  ...
  with(document.form1) {
    ...
    if (val == 'edit') {       
      if (chkstatus < 1) {   
        ...
        SetAction('service-layer-edit.php?id=' + chk_id); // edit last id.
      }                                        
    } else if (val == 'delete') {     
      if (chkstatus < 1) {   
        swal("Please select any Check Box");     
        ...
        SetAction("service-layer-action.php?do=listdelete"); 
      }                                            
    } else if (val == 'update') {                
      if (chkstatus < 1) {   
        swal("Please select any Check Box");     
        ...
        SetAction('service-layer-action.php?do=listupdate');   
      }     
    } 
				...
				<div class="main-container container-fluid">
					...
					<div class="breadcrumb-header justify-content-between">
						...
						<div class="justify-content-center mt-2">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active" aria-current="page"><?=$this->group->title_la?></li>
							</ol>
						</div>
					</div>
					<!-- /breadcrumb -->
					...
					<div class="col-lg-12">
						<div class="card custom-card">
							...
							<table  border="0" cellpadding="4" cellspacing="0">
								...
								<td><span class="txt-red">* key search by title</span></td>
								</tr>								
								</table>
							</form>
                            <div class="clear"></div>
					...
					<div class="breadcrumb-header justify-content-between">
						<div class="left-content mt-2">
							<a class="btn ripple btn-primary" href="service-layer-add.php?parent_id=<?=$this->parent_id?>"><i class="fe fe-plus me-2"></i>Add Data</a>
						</div>
						<div class="justify-content-center mt-2">
											...
											<table id="user-datatable" class="table table-bordered text-nowrap border-bottom Userlist">
												<thead>
													<tr>
														...
														<th width="12%" class="text-center">Section items</th>
														<th width="12%" class="text-center">Package items</th>
														<th width="12%" class="text-center">Data Set</th>
												...
												<tbody>
													...
													<tr>
														...
														<a class="btn ripple btn-secondary" data-bs-target="#pageLink<?=$val['id']?>" data-bs-toggle="modal" href=""><i class="las la-link"></i> view url</a>	
															
														<!-- Small modal -->
														<div class="modal fade" id="pageLink<?=$val['id']?>">
															<div class="modal-dialog" role="document">
																<div class="modal-content">
																	<div class="modal-header">
																		<h6 class="modal-title">URL Link for <?=$val['title_la']?></h6><button aria-label="Close" class="btn-close" data-bs-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
																	</div>
																	<div class="modal-body">
																		<p>content-page.php?id=<?=$val['id']?>&parent_id=<?=$val['parent_id']?></p>
																	</div>
														...
														<td><?php if($val['image'] != ''){ ?><img src="../img_service/group/<?=$val['image']?>" width="100%"><?php } ?></td>
														<td class="text-center"><a href="consumer-layer-list.php?parent_id=<?=$val['id']?>"><span class="badge badge-danger"> <?=$val['layer_amount']?> section </span></a></td>
														<td class="text-center"><a href="package-item-list.php?parent_id=<?=$val['id']?>"><span class="badge badge-danger"> <?=$val['item_amount']?> items </span></a></td>
														<td class="text-center">
															<a href="download-item-list.php?parent_id=<?=$val['id']?>"><span class="btn btn-danger-light" style="width: 100%"> Download <?=$val['file_amount']?> items </span></a><BR>
															<a href="datacloud-item-list.php?parent_id=<?=$val['id']?>"><span class="btn btn-danger-light" style="width: 100%"> Data Cloud <?=number_format($val['cloud_amount'])?> items </span></a><BR>
															<a href="leaseline-item-list.php?parent_id=<?=$val['id']?>"><span class="btn btn-danger-light" style="width: 100%"> LeaseLine <?=number_format($val['data_amount'])?> items </span></a><BR>
															<a href="gpspackage-item-list.php?parent_id=<?=$val['id']?>"><span class="btn btn-danger-light" style="width: 100%"> Gps Package <?=number_format($val['gps_amount'])?> items </span></a><BR>
															<a href="roaming-item-list.php?parent_id=<?=$val['id']?>"><span class="btn btn-danger-light" style="width: 100%"> Roaming Package <?=number_format($val['roaming_amount'])?> items </span></a><BR>
															<a href="iddcall-item-list.php?parent_id=<?=$val['id']?>"><span class="btn btn-danger-light" style="width: 100%"> IDD Call <?=number_format($val['iddcall_amount'])?> items </span></a><BR>
															<a href="wifiarea-item-list.php?parent_id=<?=$val['id']?>"><span class="btn btn-danger-light" style="width: 100%"> WIFI Area <?=number_format($val['wifiarea_amount'])?> items </span></a><BR>
															<a href="esim-item-list.php?parent_id=<?=$val['id']?>"><span class="btn btn-danger-light" style="width: 100%"> eSIM <?=number_format($val['esim_amount'])?> items </span></a>
															
														</td>
														<td>
															<span class="text-muted tx-13">
															<select class="form-select border" id="status_display<?=$val['id']?>" name="status_display<?=$val['id']?>" style="width: 100px" onchange="editStatusOnline('ServiceLayer',<?=$val['id']?>,this.value);">
														...
														<td class="text-center"><div>
															<a href="service-layer-edit.php?id=<?=$val['id']?>" class="btn btn-icon btn-info-light" data-bs-toggle="tooltip" title="" data-bs-original-title="Edit">
															<i class="las la-pen"></i></a></div>
															<div style="padding-top: 5px;">
															<a href="service-layer-action.php?do=delete&id=<?=$val['id']?>" class="btn btn-icon btn-danger-light" data-bs-toggle="tooltip" title="" data-bs-original-title="Delete">
