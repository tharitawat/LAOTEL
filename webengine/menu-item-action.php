...
switch ($gettmp["do"]) {	
	case "delete" :
		...
		$sql = "SELECT Count(id) AS amount FROM menu_items WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		
		if ($sequence < $max) {
		    $sql =  "UPDATE menu_items SET sequence = sequence - 1 WHERE sequence > ? AND parent_id = ? ";
		    $stmt = $db->Prepare($sql);
		    $rs = $db->Execute($stmt, array($sequence, $gettmp["parent_id"]));
		}
		
		if($item->Delete()){
			Lang::saveSystemLog($gettmp["id"],'Delete Banner Tabel menu_items',$_SESSION["login_admin"]["id"]);
		}
		
		$msgsuc = "ลบข้อมูลเรียบร้อย";
		$msgsuc = urlsafe_b64encode($msgsuc);
		$render = "menu-item-list.php?parent_id=".$gettmp['parent_id']."&menu_id=".$gettmp["menu_id"]."&msgsuc=".$msgsuc;
	...
	case "delete_image" :
		...
		$item->image = null;
		$item->Replace();
		Lang::saveSystemLog($gettmp["id"],'Update Image Tabel menu_items',$_SESSION["login_admin"]["id"]);
	...
	case "listdelete" :
	    ...
	    break;
	case "insert" :
		$sql = "SELECT Count(id) AS sequence FROM menu_items WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		...
		if ($item->Save()) {
			...
			$update = false;
   		    ...
   		    Lang::saveSystemLog($gettmp["id"],'Add Image Tabel menu_items',$_SESSION["login_admin"]["id"]);
	...
	//	$item->created_by = $_SESSION["login_admin"]["fullname"];
		...
		if ($item->Replace()) {
			...
			$update = false;
   		    ...
   		    Lang::saveSystemLog($gettmp["id"],'Add Image Banner Tabel menu_items',$_SESSION["login_admin"]["id"]);
			...
			$msgsuc = "บันทึกข้อมูลเรียบร้อย";
			$msgsuc = urlsafe_b64encode($msgsuc);
			$render = "menu-item-list.php?msgsuc=".$msgsuc."&menu_id=".$gettmp["menu_id"];
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		} else {
			$_SESSION["item_form"]["title_la"] = Utility::getParam("title_la");
			$_SESSION["item_form"]["title_en"] = Utility::getParam("title_en");
	...
	case "manual_sort" :
				...
				switch($gettmp['sequence']){
					case'1': 
						$sql = "UPDATE menu_items SET sort_order = ? ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt, array(1));
						
						$sql = "UPDATE menu_items SET sort_by = 'ASC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);	
					break;
					case'2': 
					...
					case'3': 
						...
						$sql = "UPDATE menu_items SET sort_by = 'DESC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);					
					break;
					case'4': 
						...
						$row = $db->getRow($stmt);	
						 ...
						 Lang::saveSystemLog($gettmp["id"],'Update Sequence Banner Tabel menu_items',$_SESSION["login_admin"]["id"]);
						...
						foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
							...
							$sql = "SELECT Count(i.id) AS amount FROM menu_items i WHERE 1=1 ";
							$stmt = $db->Prepare($sql);
							$max = $db->getOne($stmt);
							...
							if ($sequence != $gettmp["to"]) {
								...
								if ($sequence > $gettmp["to"]) {
									$sql = "UPDATE menu_items i SET i.sequence = i.sequence + 1 WHERE i.sequence >= ? AND i.sequence < ? ";
									$stmt = $db->Prepare($sql);
									$rs = $db->Execute($stmt, array($gettmp["to"], $sequence));
									$sql = "UPDATE menu_items i SET i.sequence = ? WHERE i.id = ? ";
									$stmt = $db->Prepare($sql);
