...
if ($gettmp["search_key"] != null) {
	$sql_keyword = "AND CONCAT_WS('', i.title_th,  i.subject_th) LIKE ".$db->qstr("%".Utility::encodeToDB($gettmp["search_key"])."%")." ";
}
...
$sql = "
SELECT i.*,
(SELECT Count(id) FROM article_layers t WHERE t.parent_id = i.id) AS layer_amount
FROM article_items i 
WHERE 1 = 1 
AND i.parent_id = ? 
".$sql_keyword."
".$sql_date_begin."
