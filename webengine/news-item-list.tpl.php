  ...
  with(document.form1) {
    ...
    if (val == 'edit') {       
      if (chkstatus < 1) {   
        ...
        SetAction('news-item-edit.php?id=' + chk_id); // edit last id.
      }                                        
    } else if (val == 'delete') {     
      if (chkstatus < 1) {   
        swal("Please select any Check Box");     
        ...
        SetAction("news-item-action.php?do=listdelete"); 
      }                                            
    } else if (val == 'update') {                
      if (chkstatus < 1) {   
        swal("Please select any Check Box");     
        ...
        SetAction('news-item-action.php?do=listupdate');   
      }     
    } 
			...
			<div class="main-content app-content">
				...
				<div class="main-container container-fluid">
					...
					<div class="breadcrumb-header justify-content-between">
						...
						<span class="main-content-title mg-b-0 mg-b-lg-1">NEWS & EVENT</span>
						</div>
					...
					<div class="col-lg-12">
						<div class="card custom-card">
							...
							<table  border="0" cellpadding="4" cellspacing="0">
								...
								<td><input name="search_key" type="text" class="form-control w-250" id="search_key" value="<?=$this->search_key?>"></td>
					...
					<div class="breadcrumb-header justify-content-between">
						<div class="left-content mt-2">
							<a class="btn ripple btn-primary" href="news-item-add.php"><i class="fe fe-plus me-2"></i>Add Data</a>
							<a class="btn ripple btn-primary" href="news-category-list.php"><i class="fe fe-list me-2"></i>Category</a>
							<a class="btn ripple btn-info" href="news-item-sequence.php"><i class="fe fe-list me-2"></i>Sequence</a>
							<button id="delete" class="btn btn-secondary data-table-btn me-2" onclick="checkForm('delete');">Delete</button>
						</div>
						<div class="justify-content-center mt-2">
											...
											<table id="user-datatable" class="table table-bordered text-nowrap border-bottom Userlist">
												<thead>
													<tr>
														...
														<th width="10%" class="text-center">Gallery</th>
														<th width="10%" class="text-center">Layer</th>
												...
												<tbody>
													...
													<tr>
														...
														<td>
															<p class="tx-14 font-weight-semibold text-dark mb-1"><?=substr($val['title_la'],0,100);?> ...</p>
														</td>
														<td>
														...
														<td><?php if($val['image'] != ''){ ?><img src="../img_news/<?=$val['image']?>" width="100%"><?php } ?></td>
														<td class="text-center">
														<a href="news-gallery-list.php?parent_id=<?=$val['id']?>"><span class="badge badge-danger"><?=$val['gallery_amount']?> items</span></a>
														</td>
														<td class="text-center"><a href="news-layer-list.php?parent_id=<?=$val['id']?>"><span class="badge badge-danger"> <?=$val['layer_amount']?> items </span></a></td>
														<td>
															<span class="text-muted tx-13">
															<select class="form-select border" id="status_display<?=$val['id']?>" name="status_display<?=$val['id']?>" style="width: 100px" onchange="editStatusOnline('News',<?=$val['id']?>,this.value);">
														...
														<td class="text-center"><div>
															<a href="news-item-edit.php?id=<?=$val['id']?>" class="btn btn-icon btn-info-light" data-bs-toggle="tooltip" title="" data-bs-original-title="Edit">
															<i class="las la-pen"></i></a></div>
															<div style="padding-top: 5px;">
															<a href="news-item-action.php?do=delete&id=<?=$val['id']?>" class="btn btn-icon btn-danger-light" data-bs-toggle="tooltip" title="" data-bs-original-title="Delete">
