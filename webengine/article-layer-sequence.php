...
$gettmp["group_id"] = Utility::getParam("group_id", 0);
...
$sql = "SELECT * FROM article_layers i WHERE 1=1 AND i.parent_id =  ".$db->qstr($item->id)." "; 
$stmt = $db->Prepare($sql);
$row = $db->getRow($stmt);
...
if ($gettmp["search_key"] != null) {
	$sql_keyword = "AND CONCAT_WS('', i.title_th, i.title_en) LIKE ".$db->qstr("%".Utility::encodeToDB($gettmp["search_key"])."%")." ";
}
...
$sql = "SELECT i.*, 
NULL AS dummy 
FROM article_layers i WHERE 1 = 1 AND i.parent_id = ? 
".$sql_keyword." GROUP BY i.id ORDER BY ".$sql_sort;
$stmt = $db->Prepare($sql);
...
$template->group_id = $gettmp["group_id"];
