...
$upload_path = DIR."img_info_category/";
$error_message = "";
...
switch ($gettmp["do"]) {	
	case "delete" :
		$item = new info_category();
		$item->load("id = ? ", array($gettmp["id"]));
		$gettmp["parent_id"] = $item->parent_id;
		$sequence = $item->sequence;
		
	
		$sql = "SELECT Count(id) AS amount FROM info_categories WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		
		if ($sequence < $max) {
		    $sql =  "UPDATE info_categories SET sequence = sequence - 1 WHERE sequence > ? AND parent_id = ? ";
		    $stmt = $db->Prepare($sql);
		    $rs = $db->Execute($stmt, array($sequence, $gettmp["parent_id"]));
		}
		
		if($item->Delete()){
			Lang::saveSystemLog($gettmp["id"],'Delete Banner Tabel info_categories',$_SESSION["login_admin"]["id"]);
		}
		
		$msgsuc = "ลบข้อมูลเรียบร้อย";
		$msgsuc = urlsafe_b64encode($msgsuc);
		$render = "info-category-list.php?parent_id=".$gettmp['parent_id']."&msgsuc=".$msgsuc;
	...
	case "listdelete" :
	    foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
	        ...
	        $sql = "SELECT * FROM info_categories WHERE id = ? ";
	        $stmt = $db->Prepare($sql);
	        ...
	        $sql = "DELETE FROM info_categories WHERE id = ? ";
	        $stmt = $db->Prepare($sql);
	    ...
	    break;
	case "insert" :
		$sql = "SELECT Count(id) AS sequence FROM info_categories WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		...
		if ($item->Save()) {
			...
			$update = false;
   		    ...
   		    Lang::saveSystemLog($gettmp["id"],'Add Image Tabel info_categories',$_SESSION["login_admin"]["id"]);
			...
			if ($item->sequence != $gettmp["sequence"]) {
			    ...
			    if ($item->sequence != $gettmp["sequence"]) {
			        ...
			        if ($item->sequence > $gettmp["sequence"]) {
			            $sql = "UPDATE info_categories SET sequence = sequence + 1 WHERE sequence >= ? AND sequence < ? AND parent_id = ? ";
			            $stmt = $db->Prepare($sql);
			...
			$render = "info-category-add.php?parent_id=".$gettmp['parent_id']."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	//	$item->created_by = $_SESSION["login_admin"]["fullname"];
		...
		if ($item->Replace()) {
			...
			$update = false;
   		    ...
   		    Lang::saveSystemLog($gettmp["id"],'Add Image Banner Tabel info_categories',$_SESSION["login_admin"]["id"]);

			$msgsuc = "บันทึกข้อมูลเรียบร้อย";
			$msgsuc = urlsafe_b64encode($msgsuc);
			$render = "info-category-list.php?msgsuc=".$msgsuc;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		} else {
			$_SESSION["item_form"]["title_la"] = Utility::getParam("title_la");
			$_SESSION["item_form"]["title_en"] = Utility::getParam("title_en");
			...
			$msgerr = "ไม่สามารถบันทึกข้อมูลได้ กรุณาทำรายการใหม่อีกครั้ง";
			$msgerr = urlsafe_b64encode($msgerr);
			$render = "info-category-edit.php?id=".$item->id."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	case "manual_sort" :
				...
				switch($gettmp['sequence']){
					case'1': 
						$sql = "UPDATE info_categories SET sort_order = ? ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt, array(1));
						
						$sql = "UPDATE info_categories SET sort_by = 'ASC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);	
					break;
					case'2': 
					...
					case'3': 
						...
						$sql = "UPDATE info_categories SET sort_by = 'DESC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);					
					break;
					case'4': 
						...
						$row = $db->getRow($stmt);	
						 ...
						 Lang::saveSystemLog($gettmp["id"],'Update Sequence Banner Tabel info_categories',$_SESSION["login_admin"]["id"]);
						...
						foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
							...
							$sql = "SELECT Count(i.id) AS amount FROM info_categories i WHERE 1=1 ";
							$stmt = $db->Prepare($sql);
							$max = $db->getOne($stmt);
							...
							if ($sequence != $gettmp["to"]) {
								...
								if ($sequence > $gettmp["to"]) {
									$sql = "UPDATE info_categories i SET i.sequence = i.sequence + 1 WHERE i.sequence >= ? AND i.sequence < ? ";
									$stmt = $db->Prepare($sql);
									$rs = $db->Execute($stmt, array($gettmp["to"], $sequence));
									$sql = "UPDATE info_categories i SET i.sequence = ? WHERE i.id = ? ";
									$stmt = $db->Prepare($sql);
					...
					$msgsuc = "บันทึกข้อมูลเรียบร้อย";
					$msgsuc = urlsafe_b64encode($msgsuc);
					$render = "info-category-sequence.php?parent_id=".$gettmp['parent_id']."&msgsuc=".$msgsuc;
					die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");		
