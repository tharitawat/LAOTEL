...
$group->Load("id = ? ", array($gettmp["parent_id"]));
...
$sql = "SELECT i.*, 
(SELECT Count(e.id) FROM esim_items e WHERE e.parent_id = i.id) AS esim_amount,
(SELECT Count(w.id) FROM wifiarea_items w WHERE w.parent_id = i.id) AS wifiarea_amount,
(SELECT Count(c.id) FROM iddcall_items c WHERE c.parent_id = i.id) AS iddcall_amount,
(SELECT Count(r.id) FROM roaming_items r WHERE r.parent_id = i.id) AS roaming_amount,
(SELECT Count(g.id) FROM gpspackage_items g WHERE g.parent_id = i.id) AS gps_amount,
(SELECT Count(t.id) FROM package_items t WHERE t.parent_id = i.id) AS item_amount,
(SELECT Count(d.id) FROM datacloud_items d WHERE d.parent_id = i.id) AS cloud_amount,
(SELECT Count(l.id) FROM leaseline_items l WHERE l.parent_id = i.id) AS data_amount,
(SELECT Count(f.id) FROM consumer_service_files f WHERE f.parent_id = i.id) AS file_amount,
(SELECT Count(s.id) FROM consumer_service_layers s WHERE s.parent_id = i.id) AS layer_amount 
FROM consumer_layers i WHERE 1 = 1 AND i.parent_id = ".$db->qstr($gettmp['parent_id'])." 
".$sql_keyword."
".$sql_date_begin."
".$sql_date_end."
