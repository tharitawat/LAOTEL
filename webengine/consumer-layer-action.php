...
$upload_path = DIR."img_consumer_service_layer/";
$error_message = "";
...
switch ($gettmp["do"]) {
	case "delete" :
		...
		$sql = "SELECT Count(id) AS amount FROM consumer_service_layers WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		
		if ($sequence < $max) {
		    $sql =  "UPDATE consumer_service_layers SET sequence = sequence - 1 WHERE sequence > ? AND parent_id = ? ";
		    $stmt = $db->Prepare($sql);
		...
		$render = "consumer-layer-list.php?parent_id=".$gettmp["parent_id"]."&msgsuc=".$msgsuc;
	...
	case "delete_image_la" :
		...
		$msgsuc = urlsafe_b64encode($msgsuc);
		$render = "consumer-layer-edit.php?id=".$gettmp["id"]."&msgsuc=".$msgsuc;
		die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		break;
	...
	case "delete_image_vi" :
		...
		break;	
	case "insert" :
		$sql = "SELECT Count(id) AS sequence FROM consumer_service_layers WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		...
		if ($item->Save()) {
			...
			if ($item->sequence != $gettmp["sequence"]) {
			    ...
			    if ($item->sequence != $gettmp["sequence"]) {
			        ...
			        if ($item->sequence > $gettmp["sequence"]) {
			            $sql = "UPDATE consumer_service_layers SET sequence = sequence + 1 WHERE sequence >= ? AND sequence < ? AND parent_id = ? ";
			            $stmt = $db->Prepare($sql);
			...
			$render = "consumer-layer-add.php?parent_id=".$gettmp['parent_id']."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	//	$item->created_by = $_SESSION["login_admin"]["id"];
		...
		if ($item->Replace()) {
			...
			$msgsuc = urlsafe_b64encode($msgsuc);
			$render = "consumer-layer-list.php?parent_id=".$item->parent_id."&msgsuc=".$msgsuc;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		} else {
			$_SESSION["item_form"]["description_la"] = Utility::getParam("description_la");
			$_SESSION["item_form"]["description_en"] = Utility::getParam("description_en");
			...
			$msgerr = "ไม่สามารถบันทึกข้อมูลได้ กรุณาทำรายการใหม่อีกครั้ง";
			$msgerr = urlsafe_b64encode($msgerr);
			$render = "consumer-layer-edit.php?id=".$item->id."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
		...
		case "manual_sort" :		
			#$db->debug = 1;
				switch($gettmp['sequence']){
					case'1': 
						$sql = "UPDATE consumer_service_layers SET sort_order = ? ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt, array(1));
						
						$sql = "UPDATE consumer_service_layers SET sort_by = 'ASC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);	
					break;
					case'2': 
					...
					case'3': 
						...
						$sql = "UPDATE consumer_service_layers SET sort_by = 'DESC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);					
					break;
					case'4': 
				...
				//		Lang::saveSystemLog($gettmp["id"],'Sort Retail Shop Table article_category_items By '.$gettmp['sequence'],$_SESSION["login_admin"]["id"]);
						...
						foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
							...
							$sql = "SELECT * FROM consumer_service_layers WHERE id = ? ";
							$stmt = $db->Prepare($sql);
							$row = $db->getRow($stmt, array($gettmp["id"]));
							$sequence = $row["sequence"];
						
				
							$sql = "SELECT Count(i.id) AS amount FROM consumer_service_layers i WHERE 1=1 AND i.parent_id = ? ";
							$stmt = $db->Prepare($sql);
							$max = $db->getOne($stmt, array($gettmp["parent_id"]));
							...
							if ($sequence != $gettmp["to"]) {
								...
								if ($sequence > $gettmp["to"]) {
									$sql = "UPDATE consumer_service_layers i SET i.sequence = i.sequence + 1 WHERE i.sequence >= ? AND i.sequence < ? AND i.parent_id = ? ";
									$stmt = $db->Prepare($sql);
									$rs = $db->Execute($stmt, array($gettmp["to"], $sequence,$gettmp["parent_id"]));
									$sql = "UPDATE consumer_service_layers i SET i.sequence = ? WHERE i.id = ? ";
									$stmt = $db->Prepare($sql);
