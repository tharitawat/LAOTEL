...
$sql = "SELECT * FROM consumer_categories i WHERE 1=1 "; 
$stmt = $db->Prepare($sql);
$row = $db->getRow($stmt);
...
$sql = "SELECT i.*, (SELECT Count(id) FROM consumer_groups t WHERE t.parent_id = i.id) AS item_amount 
FROM consumer_categories i WHERE 1 = 1 ORDER BY  ".$sql_sort;
$stmt = $db->Prepare($sql);
