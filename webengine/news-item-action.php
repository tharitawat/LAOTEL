...
class news_category_relate extends ADOdb_Active_Record{}
...
switch ($gettmp["do"]) {	
	case "delete" :
		...
		if ($sequence < $max) {
		    $sql =  "UPDATE news_items SET sequence = sequence - 1 WHERE sequence > ? AND parent_id = ? ";
		    $stmt = $db->Prepare($sql);
		    $rs = $db->Execute($stmt, array($sequence, $gettmp["parent_id"]));
		}
		
		if($item->Delete()){
			Lang::saveSystemLog($gettmp["id"],'Delete News Tabel news_items',$_SESSION["login_admin"]["id"]);
		}
		
		$msgsuc = "ลบข้อมูลเรียบร้อย";
		$msgsuc = urlsafe_b64encode($msgsuc);
		$render = "news-item-list.php?parent_id=".$gettmp['parent_id']."&msgsuc=".$msgsuc;
	...
	case "delete_image" :
		...
		$item->image = null;
		$item->Replace();
		Lang::saveSystemLog($gettmp["id"],'Update Image Tabel news_items',$_SESSION["login_admin"]["id"]);
		
		$msgsuc = "ลบข้อมูลเรียบร้อย";
		$msgsuc = urlsafe_b64encode($msgsuc);
		$render = "news-item-edit.php?id=".$gettmp["id"]."&msgsuc=".$msgsuc;
		die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		break;
	case "delete_thumbnail" :
		...
		$item->thumbnail = null;
		$item->Replace();
		Lang::saveSystemLog($gettmp["id"],'Update Image thumbnail Tabel news_items',$_SESSION["login_admin"]["id"]);
	...
	case "listdelete" :
	    foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
	        ...
	        $sql = "DELETE FROM news_items WHERE id = ? ";
	        $stmt = $db->Prepare($sql);
	    ...
	    break;
	case "insert" :
		$sql = "SELECT Count(id) AS sequence FROM news_items WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		...
		if ($item->Save()) {
			...
			// Set Category
			$sql = "DELETE FROM news_category_relates WHERE 1 = 1 AND parent_id = ? ";
			$stmt = $db->Prepare($sql);
			$rs = $db->Execute($stmt, array($item->id));
			$cate_id = Utility::getParam("cate_id", NULL);
			$size = sizeof($cate_id);
			$category = new news_category_relate();
			for ($i = 0; $i < $size; $i++) {
				$category->Reset();
				$category->parent_id = $item->id;
				$category->cate_id = $cate_id[$i];
				$category->Save();
			}

   		    Lang::saveSystemLog($gettmp["id"],'Add News Tabel news_items',$_SESSION["login_admin"]["id"]);
			...
			if ($item->sequence != $gettmp["sequence"]) {
			    ...
			    if ($item->sequence != $gettmp["sequence"]) {
			        ...
			        if ($item->sequence > $gettmp["sequence"]) {
			            $sql = "UPDATE news_items SET sequence = sequence + 1 WHERE sequence >= ? AND sequence < ? AND parent_id = ? ";
			            $stmt = $db->Prepare($sql);
			...
			$render = "news-item-add.php?parent_id=".$gettmp['parent_id']."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	//	$item->created_by = $_SESSION["login_admin"]["fullname"];
		...
		if ($item->Replace()) {
			...
			for ($i = 0; $i < $size; $i++) {
   		    ...
   		    Lang::saveSystemLog($gettmp["id"],'Update News Tabel news_items',$_SESSION["login_admin"]["id"]);
			...
			$msgerr = "ไม่สามารถบันทึกข้อมูลได้ กรุณาทำรายการใหม่อีกครั้ง";
			$msgerr = urlsafe_b64encode($msgerr);
			$render = "news-item-edit.php?id=".$item->id."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	case "manual_sort" :
				...
				switch($gettmp['sequence']){
					case'1': 
						$sql = "UPDATE news_items SET sort_order = ? ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt, array(1));
						
						$sql = "UPDATE news_items SET sort_by = 'ASC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);	
					break;
					case'2': 
					...
					case'3': 
						...
						$sql = "UPDATE news_items SET sort_by = 'DESC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);					
					break;
					case'4': 
						...
						$row = $db->getRow($stmt);	
						 ...
						 Lang::saveSystemLog($gettmp["id"],'Update Sequence Banner Tabel news_items',$_SESSION["login_admin"]["id"]);
						...
						foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
							...
							$sql = "SELECT Count(i.id) AS amount FROM news_items i WHERE 1=1 ";
							$stmt = $db->Prepare($sql);
							$max = $db->getOne($stmt);
							...
							if ($sequence != $gettmp["to"]) {
								...
								if ($sequence > $gettmp["to"]) {
									$sql = "UPDATE news_items i SET i.sequence = i.sequence + 1 WHERE i.sequence >= ? AND i.sequence < ? ";
									$stmt = $db->Prepare($sql);
									$rs = $db->Execute($stmt, array($gettmp["to"], $sequence));
									$sql = "UPDATE news_items i SET i.sequence = ? WHERE i.id = ? ";
									$stmt = $db->Prepare($sql);
					...
					$msgsuc = "บันทึกข้อมูลเรียบร้อย";
					$msgsuc = urlsafe_b64encode($msgsuc);
					$render = "news-item-sequence.php?msgsuc=".$msgsuc;
					die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");		
