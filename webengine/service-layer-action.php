...
$upload_path = DIR."img_service/group/";
$error_message = "";
...
switch ($gettmp["do"]) {	
	case "delete" :
		...
		$sql = "SELECT Count(id) AS amount FROM consumer_layers WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		
		if ($sequence < $max) {
		    $sql =  "UPDATE consumer_layers SET sequence = sequence - 1 WHERE sequence > ? AND parent_id = ? ";
		    $stmt = $db->Prepare($sql);
		    $rs = $db->Execute($stmt, array($sequence, $gettmp["parent_id"]));
		}
		
		if($item->Delete()){
			Lang::saveSystemLog($gettmp["id"],'Delete News Tabel consumer_layers',$_SESSION["login_admin"]["id"]);
		}
		
		$msgsuc = "ลบข้อมูลเรียบร้อย";
		$msgsuc = urlsafe_b64encode($msgsuc);
		$render = "service-layer-list.php?parent_id=".$gettmp['parent_id']."&msgsuc=".$msgsuc;
	...
	case "delete_image" :
		...
		$item->image = null;
		$item->Replace();
		Lang::saveSystemLog($gettmp["id"],'Update Image Tabel consumer_layers',$_SESSION["login_admin"]["id"]);
		
		$msgsuc = "ลบข้อมูลเรียบร้อย";
		$msgsuc = urlsafe_b64encode($msgsuc);
		$render = "service-layer-edit.php?id=".$gettmp["id"]."&msgsuc=".$msgsuc;
		die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		break;
	case "delete_thumbnail" :
		...
		$item->thumbnail = null;
		$item->Replace();
		Lang::saveSystemLog($gettmp["id"],'Update Image thumbnail Tabel consumer_layers',$_SESSION["login_admin"]["id"]);
		...
		die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		break;
	case "delete_thumbnail_youtube" :
		$item = new consumer_layer();
		$item->load("id = ? ", array($gettmp["id"]));
		
		Utility::deleteFile($upload_path."", $item->thumbnail_youtube);
		Utility::deleteFile($upload_path."cover/", $item->thumbnail_youtube);
		$item->thumbnail_youtube = null;
		$item->Replace();
		Lang::saveSystemLog($gettmp["id"],'Update Image thumbnail _youtube consumer_layers',$_SESSION["login_admin"]["id"]);
		...
		die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		break;		
	case "delete_image_icon" :
		$item = new consumer_layer();
		$item->load("id = ? ", array($gettmp["id"]));
		
		Utility::deleteFile($upload_path."", $item->image_icon);
		Utility::deleteFile($upload_path."thumbnail/", $item->image_icon);
		$item->image_icon = null;
		$item->Replace();
		Lang::saveSystemLog($gettmp["id"],'Update image_icon thumbnail Tabel consumer_layers',$_SESSION["login_admin"]["id"]);
	...
	case "listdelete" :
	    foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
	        ...
	        $sql = "SELECT * FROM consumer_layers WHERE id = ? ";
	        $stmt = $db->Prepare($sql);
	        ...
	        $sql = "DELETE FROM consumer_layers WHERE id = ? ";
	        $stmt = $db->Prepare($sql);
	    ...
	    break;
	case "insert" :
		$sql = "SELECT Count(id) AS sequence FROM consumer_layers WHERE parent_id = ? ";
		$stmt = $db->Prepare($sql);
		$max = $db->getOne($stmt, array($gettmp["parent_id"]));
		...
		$item->form_request = Utility::getParam("form_request",'0');	
		
		if ($item->Save()) {
			unset($_SESSION["item_form"]);
			$update = false;

   		    Lang::saveSystemLog($gettmp["id"],'Add News Tabel consumer_layers',$_SESSION["login_admin"]["id"]);
			...
			if ($_FILES["image"] != null) {
				...
				if ($upload->uploaded) {
					...
					$file_new_name_body = $item->id."-img-".rand(1000, 9999);
			...
			// Set icon
			if ($_FILES["image_icon"] != null) {
				$file_temp = $item->image_icon;
				$upload = new upload($_FILES["image_icon"]);
				if ($upload->uploaded) {
					
					$image_name = explode(".", $_FILES["image_icon"]["name"]);
					$file_new_name_body = $item->id."-icon-".rand(1000, 9999);
					...
					if ($upload->processed) {
						...
						$update = true;
						$item->image_icon = $upload->file_dst_name;
			...
			// Set thumbnail_youtube 
			if ($_FILES["thumbnail_youtube"] != null) {
				$file_temp = $item->thumbnail_youtube;
				$upload = new upload($_FILES["thumbnail_youtube"]);
				if ($upload->uploaded) {
					
					$image_name = explode(".", $_FILES["thumbnail_youtube"]["name"]);
					$file_new_name_body = $item->id."-cover-".rand(1000, 9999);
				...
				//	$upload->jpeg_quality = 100;
					...
					if ($upload->processed) {
						...
						$update = true;
						$item->thumbnail_youtube = $upload->file_dst_name;
			...
			if ($item->sequence != $gettmp["sequence"]) {
			    ...
			    if ($item->sequence != $gettmp["sequence"]) {
			        ...
			        if ($item->sequence > $gettmp["sequence"]) {
			            $sql = "UPDATE consumer_layers SET sequence = sequence + 1 WHERE sequence >= ? AND sequence < ? AND parent_id = ? ";
			            $stmt = $db->Prepare($sql);
			...
			$render = "service-layer-add.php?parent_id=".$gettmp['parent_id']."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	//	$item->created_by = $_SESSION["login_admin"]["fullname"];
		...
		if ($item->Replace()) {
			...
			$update = false;
   		    ...
   		    Lang::saveSystemLog($gettmp["id"],'Update News Tabel consumer_layers',$_SESSION["login_admin"]["id"]);
			...
			$msgerr = "ไม่สามารถบันทึกข้อมูลได้ กรุณาทำรายการใหม่อีกครั้ง";
			$msgerr = urlsafe_b64encode($msgerr);
			$render = "service-layer-edit.php?id=".$item->id."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	case "manual_sort" :
				...
				switch($gettmp['sequence']){
					case'1': 
						$sql = "UPDATE consumer_layers SET sort_order = ? ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt, array(1));
						
						$sql = "UPDATE consumer_layers SET sort_by = 'ASC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);	
					break;
					case'2': 
					...
					case'3': 
						...
						$sql = "UPDATE consumer_layers SET sort_by = 'DESC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);					
					break;
					case'4': 
						...
						$row = $db->getRow($stmt);	
						 ...
						 Lang::saveSystemLog($gettmp["id"],'Update Sequence Banner Tabel consumer_layers',$_SESSION["login_admin"]["id"]);
						...
						foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
							...
							$sql = "SELECT Count(i.id) AS amount FROM consumer_layers i WHERE 1=1 ";
							$stmt = $db->Prepare($sql);
							$max = $db->getOne($stmt);
							...
							if ($sequence != $gettmp["to"]) {
								...
								if ($sequence > $gettmp["to"]) {
									$sql = "UPDATE consumer_layers i SET i.sequence = i.sequence + 1 WHERE i.sequence >= ? AND i.sequence < ? ";
									$stmt = $db->Prepare($sql);
									$rs = $db->Execute($stmt, array($gettmp["to"], $sequence));
									$sql = "UPDATE consumer_layers i SET i.sequence = ? WHERE i.id = ? ";
									$stmt = $db->Prepare($sql);
					...
					$msgsuc = "บันทึกข้อมูลเรียบร้อย";
					$msgsuc = urlsafe_b64encode($msgsuc);
					$render = "service-layer-sequence.php?msgsuc=".$msgsuc;
					die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");		
