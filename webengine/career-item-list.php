...
$cate->Load("id = ? ", array($gettmp["parent_id"]));
$template->cate = $cate;
...
$sql = "SELECT i.*, (SELECT Count(id) FROM career_layers t WHERE t.parent_id = i.id) AS layer_amount
FROM career_items i WHERE 1 = 1 AND i.parent_id = ".$db->qstr($gettmp['parent_id'])." 
".$sql_keyword."
".$sql_date_begin."
".$sql_date_end."
