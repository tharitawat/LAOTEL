...
$sql = "SELECT i.*, (SELECT Count(id) FROM groupmenu_items t WHERE t.parent_id = i.id) AS menu_amount FROM submenu_items i WHERE 1 = 1  AND i.parent_id = ".$db->qstr($gettmp['parent_id'])." ".$sql_keyword."ORDER BY i.sequence ASC, i.id ASC";
$stmt = $db->Prepare($sql);
