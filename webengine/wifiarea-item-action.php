...
$upload_path = DIR."img_wifiarea_item/";
$error_message = "";
...
switch ($gettmp["do"]) {	
	case "delete" :
		...
		if ($sequence < $max) {
		    $sql =  "UPDATE wifiarea_items SET sequence = sequence - 1 WHERE sequence > ? AND parent_id = ? ";
		    $stmt = $db->Prepare($sql);
		    $rs = $db->Execute($stmt, array($sequence, $gettmp["parent_id"]));
		}
		
		if($item->Delete()){
			Lang::saveSystemLog($gettmp["id"],'Delete Icon Tabel wifiarea_items',$_SESSION["login_admin"]["id"]);
		}
		
		$msgsuc = "ลบข้อมูลเรียบร้อย";
		$msgsuc = urlsafe_b64encode($msgsuc);
		$render = "wifiarea-item-list.php?parent_id=".$gettmp['parent_id']."&msgsuc=".$msgsuc;
	...
	case "listdelete" :
	    foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
	        ...
	        $sql = "SELECT * FROM wifiarea_items WHERE id = ? ";
	        $stmt = $db->Prepare($sql);
			...
			Lang::saveSystemLog($gettmp["id"],'Delete Item Tabel wifiarea_items',$_SESSION["login_admin"]["id"]);
        
	        $sql = "DELETE FROM wifiarea_items WHERE id = ? ";
	        $stmt = $db->Prepare($sql);
	...
	case "insert" :
		...
		$item->location = Utility::encodeToDB(Utility::getParam("location"));
		$item->number_point = Utility::encodeToDB(Utility::getParam("number_point"));
		$item->indoor_point = Utility::encodeToDB(Utility::getParam("indoor_point"));
		$item->outdoor_point = Utility::encodeToDB(Utility::getParam("outdoor_point"));
		...
		if ($item->Save()) {
			...
			$update = false;
   		    ...
   		    Lang::saveSystemLog($gettmp["id"],'Add wifiarea_items',$_SESSION["login_admin"]["id"]);
			...
			$_SESSION["item_form"]["location"] = Utility::getParam("location");
			$_SESSION["item_form"]["number_point"] = Utility::getParam("number_point");
			$_SESSION["item_form"]["indoor_point"] = Utility::getParam("indoor_point");
			$_SESSION["item_form"]["outdoor_point"] = Utility::getParam("outdoor_point");
			...
			$render = "wifiarea-item-add.php?parent_id=".$gettmp['parent_id']."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	case "update" :
		...
		if ($item->Replace()) {
			...
			$update = false;
   		    ...
   		    Lang::saveSystemLog($gettmp["id"],'Update item wifiarea_items',$_SESSION["login_admin"]["id"]);
			...
			$msgerr = "ไม่สามารถบันทึกข้อมูลได้ กรุณาทำรายการใหม่อีกครั้ง";
			$msgerr = urlsafe_b64encode($msgerr);
			$render = "wifiarea-item-edit.php?id=".$item->id."&msgerr=".$msgerr;
			die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");
		}
	...
	case "manual_sort" :
				...
				switch($gettmp['sequence']){
					case'1': 
						$sql = "UPDATE wifiarea_items SET sort_order = ? ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt, array(1));
						
						$sql = "UPDATE wifiarea_items SET sort_by = 'ASC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);	
					break;
					case'2': 
					...
					case'3': 
						...
						$sql = "UPDATE wifiarea_items SET sort_by = 'DESC' ";
						$stmt = $db->Prepare($sql);
						$row = $db->getRow($stmt);					
					break;
					case'4': 
						...
						$row = $db->getRow($stmt);	
						 ...
						 Lang::saveSystemLog($gettmp["id"],'Update Sequence Tabel wifiarea_items',$_SESSION["login_admin"]["id"]);
						...
						foreach (Utility::getParam("chkbox") as $row => $gettmp["no"]) {
							...
							$sql = "SELECT Count(i.id) AS amount FROM wifiarea_items i WHERE 1=1 ";
							$stmt = $db->Prepare($sql);
							$max = $db->getOne($stmt);
							...
							if ($sequence != $gettmp["to"]) {
								...
								if ($sequence > $gettmp["to"]) {
									$sql = "UPDATE wifiarea_items i SET i.sequence = i.sequence + 1 WHERE i.sequence >= ? AND i.sequence < ? ";
									$stmt = $db->Prepare($sql);
									$rs = $db->Execute($stmt, array($gettmp["to"], $sequence));
									$sql = "UPDATE wifiarea_items i SET i.sequence = ? WHERE i.id = ? ";
									$stmt = $db->Prepare($sql);
					...
					$msgsuc = "บันทึกข้อมูลเรียบร้อย";
					$msgsuc = urlsafe_b64encode($msgsuc);
					$render = "wifiarea-item-sequence.php?parent_id=".$gettmp['parent_id']."&msgsuc=".$msgsuc;
					die("<meta http-equiv=\"refresh\" content=\"0; URL=$render\">");		
